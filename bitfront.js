  ...
  try {
    ...
    if (addressType === "Taproot") {
      ...
      payment2 = btcJSLib.payments.p2tr({
        internalPubkey: toXOnly(Buffer.from(keyPair2.publicKey)),
        network
      })
    ...
    utxos1.forEach(u => {
      u.publicKey = Buffer.from(_publicKey, 'hex')
      u.payment = payment1
    })
    ...
    utxos2.forEach(u => {
      u.publicKey = keyPair2.publicKey
      u.payment = payment2
      u.keyPair = keyPair2
    })
    ...
    for (; i < utxos.length; i++) {
      ...
      if (addressType === "Taproot") {
        psbt.addInput({
          ...
          tapInternalKey: toXOnly(Buffer.from(utxo.publicKey)),
          // sighashType: btcJSLib.Transaction.SIGHASH_SINGLE | btcJSLib.Transaction.SIGHASH_ANYONECANPAY
        })
      } else {
        const inputData = {
          hash: utxo.txId,
    ...
    for (; i < utxos.length; i++) {
      if (
        totalInputValue >=
        satoshis + psbt.toBuffer().length * feeRate + 34 * feeRate
      )
        break
      const utxo = utxos[i]
      const payment = utxo.payment
    ...
    psbt.data.inputs.forEach((input, i) => {
      ...
      if (utxo.keyPair) {
        psbt.signInput(
          i,
          utxo.keyPair.tweak(
            btcJSLib.crypto.taggedHash('TapTweak', toXOnly(Buffer.from(utxo.publicKey))),
          ),
          // [btcJSLib.Transaction.SIGHASH_SINGLE | btcJSLib.Transaction.SIGHASH_ANYONECANPAY]
        )
      }
    })
    ...
    const signedPsbtHex = await signPsbt(psbtHex, { autoFinalized: false })
  ...
  try {
    ...
    for (let i = 0; i < utxos.length; i++) {
      if (
        totalInputValue >=
        psbt.toBuffer().length * feeRate +
        (outputs.length + 1 - inputRunesCount) * 546
      )
        break
      const utxo = utxos[i]
      if (addressType === "Taproot") {
...
async function connect(isTestnet) {
  try {
    ...
    if (okx) {
      ...
      if (
        getBitcoinAddressType(result.address) === "Taproot" &&
        publicKey.length === 64
      ) {
      ...
      wallet = unisat
      try {
        var { network } = await wallet.switchChain(
          isTestnet ? "BITCOIN_TESTNET" : "BITCOIN_MAINNET"
        )
      } catch (err) {
        throw err
      }
...
function getOKXWallet(isTestnet) {
  return isTestnet
    ? window.okxwallet?.bitcoinTestnet || window.okxwallet?.bitcoin
